...
/* ==========================================
   SAFETY POLYFILLS — Assurance fonctionnement
   ========================================== */

// S'assurer que Modal est disponible et fonctionne
// (défini normalement dans router.js — polyfill de secours)
window._modalSafetyCheck = function() {
  if (typeof Modal === 'undefined' || !Modal.open) {
    console.warn('[Horizon] Modal non trouvé — utilisation du polyfill');
    window.Modal = {
      open: function(name) {
        ...
        if (modal)   { modal.classList.add('open'); modal.style.transform='translateX(-50%) translateY(0)'; modal.style.pointerEvents='auto'; }
      },
      close: function(name) {
        const overlay = document.getElementById('overlay-' + name);
        const modal   = document.getElementById('modal-' + name);
        if (overlay) { overlay.classList.remove('open'); overlay.style.opacity=''; overlay.style.pointerEvents=''; }
        if (modal)   { modal.classList.remove('open'); modal.style.transform=''; modal.style.pointerEvents=''; }
      }
    };
  }

  // S'assurer que Router est disponible
  if (typeof Router === 'undefined' || !Router.go) {
    console.warn('[Horizon] Router non trouvé — utilisation du polyfill');
    window._routerHandlers = window._routerHandlers || {};
    window.Router = {
      _current: '',
      go: function(name, opts) {
        ...
        const handler = window._routerHandlers[name];
        if (handler) { handler(opts?.params || {}); }
      },
      back: function() { window.history.back(); },
      onEnter: function(name, fn) { window._routerHandlers[name] = fn; },
      current: function() { return this._current; }
    };
  }

  // S'assurer que Toast est disponible
    ...
    window.Toast = {
      _show: function(msg, color) {
        ...
        el.style.cssText = `position:fixed;top:20px;left:50%;transform:translateX(-50%);
          ...
          transition:opacity .3s;max-width:300px;text-align:center`;
        el.textContent = msg;
        document.body.appendChild(el);
        setTimeout(() => { el.style.opacity='0'; setTimeout(()=>el.remove(),300); }, 2500);
      },
      success: function(m) { this._show(m, '#2d6a4f'); },
      error:   function(m) { this._show(m, '#c0392b'); },
      warning: function(m) { this._show(m, '#d4810a'); },
      show:    function(m) { this._show(m, '#333'); },
    };
  }
};

// Exécuter le check le plus tôt possible
document.addEventListener('DOMContentLoaded', () => {
  // Attendre que router.js se charge (il est chargé avant app.js dans le HTML)
  window._modalSafetyCheck();
});
...
async function verifierNouveautes() {
  const derniereVue = localStorage.getItem('horizon_version_vue') || '0.0.0';
...
function afficherNouveautes(fromParam = false) {
  ...
  const derniereVue = fromParam ? '0.0.0' : (localStorage.getItem('horizon_version_vue') || '0.0.0');
  ...
  localStorage.setItem('horizon_version_vue', APP_VERSION);
}
...
window.addEventListener('DOMContentLoaded',async()=>{
  ...
  // Sinon → écran activation déjà visible par défaut
});
