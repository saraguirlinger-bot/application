...
/* ── Parsing montant sécurisé (virgule, espaces, null) ── */
...
async function appInit() {
  try {
    ...
    console.error('[appInit] DB échec:', e);
    _afficherErreurCritique('Base de données inaccessible. Rechargez la page.');
    return;
  }
  try {
  ...
  // Onboarding supprimé — l'app démarre directement
  // Marquer l'onboarding comme terminé pour les utilisateurs existants
  try { await D().markOnboardingDone(); } catch(e) {}
  startApp();
}
...
async function startApp() {
  ...
  const key = _safeStorage.get('horizon_licence_key', null);
  txt('param-licence-val', key ? key+' · Active' : 'Non activée');
  // Vérifier les nouveautés après chargement
  setTimeout(verifierNouveautes, 1500);
  // Vérifier si une version plus récente est disponible
  setTimeout(verifierVersionDistante, 2000);
}
...
const CHANGELOG = [
  {
    version: '3.6.0',
    date: '2026-06',
    titre: 'Accès direct au tableau de bord',
    nouveautes: [
      'Suppression du paramétrage obligatoire au démarrage',
      'Accès direct au tableau de bord à l\'ouverture',
      'Correction safe-area iPhone — boutons accessibles',
  ...
  },
  {
    version: '3.5.0',
    date: '2026-06',
    titre: 'Purge cache & stabilisation mobile',
    nouveautes: [
      'Purge forcée des anciens caches iOS',
      'Fix écran noir sur téléphone',
      'Saisie montants au clavier natif',
    ]
  },
  {
    version: '3.4.0',
    date: '2026-06',
    titre: 'Correction mobile & saisie',
    nouveautes: [
      'Fix écrans noirs sur téléphone',
      'Saisie des montants au clavier natif',
      'Suppression des numpads visuels',
      'Fix boutons iOS Safari',
      'Harmonisation version 3.4',
    ]
  },
  {
    version: '3.3.0',
    date: '2026-06',
    titre: 'Stabilisation PWA & corrections iOS',
    nouveautes: [
      'Correction système de mise à jour — cache v3.3',
      'Purge automatique des anciens caches iPhone',
      'Correction boutons non cliquables sur iOS Safari',
      'Stabilisation PWA — manifest et chemins relatifs',
      'Harmonisation des versions sur tous les fichiers',
      'Fix screen-onboarding qui bloquait les clics',
      'Amélioration du fix.js pour tous les boutons',
    ]
  },
  {
    version: '3.2.0',
    date: '2026-06',
    titre: 'Mise à jour majeure budgétaire',
    nouveautes: [
      ...
      'Calendrier des prélèvements dans les statistiques',
      'Alertes anticipées intelligentes',
      'Onboarding revu — explication claire des 3 concepts',
    ]
  },
  {
    version: '3.1.0',
    date: '2026-05',
    titre: 'Améliorations budgétaires',
    nouveautes: [
      'Couleurs pastels pour les enveloppes et transactions',
      'Rapprochement bancaire 3 états 🟢🟡🔴',
      'Charges variables paramétrables',
      'Clôture de mois avec réaffectation des reliquats',
    ]
  },
];
...
// Vérifier si une nouvelle version est disponible sur le serveur
async function verifierVersionDistante() {
  try {
    // Fetch version.json sans cache
    const res = await fetch('./version.json?t=' + Date.now(), { cache: 'no-store' });
    if (!res.ok) return null;
    const data = await res.json();

    const versionDistante = data.version;
    const versionLocale   = APP_VERSION;
    const cacheDistant    = data.cache;

    // Mode debug
    console.log('[Version] App locale :', versionLocale);
    console.log('[Version] Version distante :', versionDistante);
    console.log('[Version] Cache distant :', cacheDistant);

    // Récupérer la version du SW actif
    if ('serviceWorker' in navigator && navigator.serviceWorker.controller) {
      navigator.serviceWorker.controller.postMessage({ type: 'GET_VERSION' });
    }

    // Comparer les versions
    if (versionDistante && versionDistante !== versionLocale) {
      console.log('[Version] Nouvelle version détectée :', versionDistante);
      afficherBandeauMiseAJour(versionDistante, data.notes || '');
      return { nouvelle: true, version: versionDistante };
    }

    return { nouvelle: false, version: versionLocale };
  } catch(e) {
    console.warn('[Version] Impossible de vérifier la version distante:', e.message);
    return null;
  }
}

// Afficher le bandeau de mise à jour
function afficherBandeauMiseAJour(nouvelleVersion, notes) {
  // Éviter les doublons
  ...
  banner.style.cssText = [
    ...
    'background:#1a3d2a', 'border:1px solid rgba(74,222,128,0.3)',
  ...
  banner.innerHTML = `
    <div style="flex:1">
      <div style="font-size:.75rem;font-weight:700;color:#4ade80">
        🔄 Version ${nouvelleVersion} disponible
      </div>
      <div style="font-size:.6rem;color:rgba(255,255,255,.5);margin-top:2px">
        ${notes || 'Nouvelle mise à jour disponible'}
      </div>
    </div>
    <div onclick="appliquerMiseAJour()"
         style="padding:8px 14px;background:rgba(74,222,128,0.15);border:1px solid rgba(74,222,128,0.3);
                border-radius:10px;font-size:.72rem;font-weight:700;color:#4ade80;cursor:pointer;
                white-space:nowrap;touch-action:manipulation">
      Mettre à jour
    </div>
    <div onclick="this.parentNode.remove()"
         style="font-size:.8rem;color:rgba(255,255,255,.3);cursor:pointer;padding:4px;
...
// Appliquer la mise à jour : purger le cache et recharger
async function appliquerMiseAJour() {
  ...
  // 2. Forcer le SW à prendre le contrôle immédiatement
  ...
  // 3. Recharger avec timestamp pour contourner le cache OVH
  window.location.href = window.location.href.split('?')[0]
    + '?v=3.6.0&t=' + Date.now();
}
window.appliquerMiseAJour = appliquerMiseAJour;

// Écouter les messages du SW (version reçue)
if ('serviceWorker' in navigator) {
  navigator.serviceWorker.addEventListener('message', (event) => {
    if (event.data?.type === 'SW_VERSION') {
      console.log('[Version] SW actif :', event.data.version, '| Cache :', event.data.cache);
    }
  });
}
...
async function verifierNouveautes() {
  const derniereVue = _safeStorage.get('horizon_version_vue', '0.0.0') || '0.0.0';
  if(derniereVue === APP_VERSION) return; // Déjà vu
  // Afficher les nouveautés après un délai (laisser l'app se charger)
  setTimeout(() => afficherNouveautes(false), 1200);
}

function afficherNouveautes(fromParam = false) {
  const el = $('modal-nouveautes');
  const content = $('nouveautes-content');
  if(!el || !content) return;

  // Entrées à afficher : tout ce qui est > version déjà vue
  const derniereVue = fromParam ? '0.0.0' : (_safeStorage.get('horizon_version_vue', '0.0.0') || '0.0.0');
  const nouvelles = fromParam ? CHANGELOG : CHANGELOG.filter(c => c.version > derniereVue);
  if(!nouvelles.length && !fromParam) return;

  content.innerHTML = nouvelles.map((c,i) => `
    <div style="margin-bottom:${i<nouvelles.length-1?'16px':'0'}">
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
        <div style="font-size:.6rem;font-weight:600;padding:3px 9px;background:rgba(74,222,128,0.1);border:1px solid rgba(74,222,128,0.2);border-radius:100px;color:#4ade80">v${h(c.version)}</div>
        <div style="font-size:.6rem;color:var(--text3)">${h(c.date)}</div>
      </div>
      <div style="font-size:.8rem;font-weight:700;color:var(--text);margin-bottom:8px">${h(c.titre)}</div>
      <div style="display:flex;flex-direction:column;gap:5px">
        ${c.nouveautes.map(n=>`
          <div style="display:flex;gap:8px;align-items:flex-start">
            <span style="color:#4ade80;flex-shrink:0;margin-top:1px">✓</span>
            <span style="font-size:.7rem;color:var(--text2);line-height:1.4">${h(n)}</span>
          </div>`).join('')}
      </div>
    </div>`).join('<div style="height:1px;background:var(--noir-border);margin:12px 0"></div>');

  Modal.open('nouveautes');
  _safeStorage.set('horizon_version_vue', APP_VERSION);
}
window.afficherNouveautes = afficherNouveautes;

function fermerNouveautes() { Modal.close('nouveautes'); }
window.fermerNouveautes = fermerNouveautes;
...
const ONB_META = {
  intro:      { emoji:'__LOGO__', titre:'Bienvenue sur Horizon Budget', sub:'Configurons ensemble votre budget en quelques étapes.' },
  ...
  enveloppes: { emoji:'🎯', titre:'Enveloppes budget', sub:'Alimentation, carburant, loisirs… Budget mensuel remis à zéro chaque mois.', color:'#a7d2bf', colorBg:'rgba(167,210,191,0.1)', conseil:'💡 Ex : Alimentation 400€ · Carburant 150€ · Loisirs 100€ — Le solde non dépensé peut être transféré vers une cagnotte en fin de mois.' },
...
function showOnboarding() {
  // Masquer #app (et sa bottom-nav) pendant l'onboarding
  ...
  if(s.type==='intro') {
    html=`
      <div style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:0 8px">
        ${ m.emoji === '__LOGO__'
          ? `<svg width="64" height="64" viewBox="0 0 60 60" fill="none" style="margin-bottom:18px;display:block;margin-left:auto;margin-right:auto"><circle cx="30" cy="30" r="27" stroke="#2d6a4f" stroke-width="2.5" fill="none"/><circle cx="30" cy="30" r="18" stroke="#2d6a4f" stroke-width="2.5" fill="none"/><circle cx="30" cy="30" r="10" stroke="#2d6a4f" stroke-width="2.5" fill="none"/><circle cx="30" cy="30" r="4" fill="#2d6a4f"/></svg>`
...
/* ── Ajout via prompt ── */
function onbAddC() {
  const n=prompt('Nom de la charge fixe (ex: Loyer, EDF…)'); if(!n||!n.trim())return;
  const mStr=prompt('Montant mensuel en € (ex: 750)'); if(mStr===null)return;
  const m=parseFloat(mStr.replace(',','.')); if(!m||m<=0){ Toast.warning('Montant invalide'); return; }
  onbData.charges.push({nom:n.trim(),montant:m,categorie:'Charges fixes',actif:true});
  renderOnbC();
}
function onbAddE() {
  const n=prompt("Nom de l'enveloppe (ex: Alimentation, Loisirs…)"); if(!n||!n.trim())return;
  const mStr=prompt('Budget mensuel en € (ex: 400)'); if(mStr===null)return;
  const m=parseFloat(mStr.replace(',','.')); if(!m||m<=0){ Toast.warning('Montant invalide'); return; }
...
async function saveOnboarding() {
  ...
  // Réafficher #app avec la bottom-nav
  const appEl=$('app'); if(appEl) appEl.style.display='flex';
...
async function chargerAccueil() {
  ...
  const updateTime=()=>{
    const n=new Date();
    const h=n.getHours();
    const m=n.getMinutes()<10?'0'+n.getMinutes():''+n.getMinutes();
    const timeStr=h+':'+m;
  ...
  // Afficher la version en bas de l'écran d'accueil
  const verEl=document.getElementById('accueil-version');
...
Router.onEnter('saisie', async function() {
  ...
  // Input direct — pas de Numpad
  const amInput=$('saisie-amount-input');
  if(amInput){ amInput.value=''; amInput.style.color='#a7c3e6'; setTimeout(()=>amInput.focus(),300); }
...
function setSaisieTypeUI(type) {
  ...
  const amInput=$('saisie-amount-input');
  if(amInput) amInput.style.color=col;
  if(am)am.style.color=col; if(cur && cur.style) cur.style.background=col;
  updateBudgetRow();
}
...
async function doSave() {
  const amInput=$('saisie-amount-input');
  const montant=parseMontant(amInput?.value);
  if(montant<=0){ Toast.error('Indiquez un montant'); if(amInput)amInput.focus(); return; }
  const libEl=$('saisie-libelle-val');
  ...
  try { await detecterRecurrente(libelle,montant); } catch(e) {}
...
window.promptSoldeInit=async function(){
  ...
  const n=parseFloat(String(v).replace(/\s/g,'').replace(',','.')); if(isNaN(n)){Toast.error('Montant invalide');return;}
...
window.promptRevenus = function() { Router.go('revenus'); };

/* ==========================================
   ÉCRAN REVENUS MULTI-SOURCES
   ========================================== */

const CATS_REVENUS = ["Salaire", "Auto-entrepreneur", "Retraite", "Pension alimentaire", "CAF / Allocations", "Invalidité / AAH", "Indemnités chômage", "Remboursement assurance", "Aide ponctuelle / Sinistre", "Autre"];

Router.onEnter('revenus', renderRevenus);

async function renderRevenus() {
  const revenus = await D().getRevenus();
  const total   = revenus.filter(r=>r.actif!==false).reduce((s,r)=>s+(parseFloat(r.montant)||0),0);

  const el = $('revenus-list');
  const elTotal = $('revenus-total');
  if (elTotal) elTotal.textContent = fmt(total) + ' / mois';

  if (!el) return;
  if (!revenus.length) {
    el.innerHTML = `<div class="empty-state">
      <div class="empty-icon">💶</div>
      <div class="empty-title">Aucun revenu enregistré</div>
      <div class="empty-sub">Ajoutez vos sources de revenus</div>
    </div>`;
    return;
  }
  el.innerHTML = revenus.map(r => `
    <div class="card-row" style="opacity:${r.actif===false?'.45':'1'}">
      <div class="row-icon" style="background:rgba(167,210,191,0.12);border:1px solid rgba(167,210,191,0.25);font-size:.9rem">💶</div>
      <div class="row-info" style="flex:1">
        <div class="row-label">${h(r.nom)}</div>
        <div class="row-sub" style="color:var(--text3)">${h(r.categorie||'Autre')}</div>
      </div>
      <div class="row-value" style="color:#a7d2bf;font-weight:700">${fmt(r.montant)}</div>
      <div onclick="ouvrirEditRevenu(${r.id})" style="padding:6px;cursor:pointer;color:var(--text3);font-size:.75rem;margin-left:4px">✏️</div>
      <div onclick="supprimerRevenu(${r.id})" style="padding:6px;cursor:pointer;color:var(--text3);font-size:.75rem">🗑</div>
    </div>`).join('');
}
...
async function supprimerRevenu(id) {
  ...
  await D().deleteRevenu(id);
  Toast.success('Revenu supprimé');
  renderRevenus();
}
window.supprimerRevenu = supprimerRevenu;

function ouvrirNewRevenu() {
  const nomEl     = document.getElementById('rev-nom');
  const montantEl = document.getElementById('rev-montant');
  if (nomEl)     nomEl.value     = '';
  if (montantEl) montantEl.value = '';
  // Peupler le select catégories
  const catEl = document.getElementById('rev-cat-select');
  if (catEl) {
    const cats = typeof CATS_REVENUS !== 'undefined' ? CATS_REVENUS :
      ['Salaire','Auto-entrepreneur','Retraite','Pension alimentaire',
       'CAF / Allocations','Invalidité / AAH','Indemnités chômage',
       'Remboursement assurance','Aide ponctuelle / Sinistre','Autre'];
    catEl.innerHTML = cats.map(c => `<option value="${c}">${c}</option>`).join('');
  }
  Modal.open('newRevenu');
  setTimeout(() => nomEl?.focus(), 300);
}
window.ouvrirNewRevenu = ouvrirNewRevenu;

function ouvrirEditRevenu(id) {
  D().getRevenus().then(list => {
    const r = list.find(x => x.id === id);
    if (!r) return;
    $('edit-rev-id').value   = id;
    $('edit-rev-nom').value  = r.nom;
    $('edit-rev-montant').value = r.montant;
    renderCatRevenus(r.categorie, true);
    Modal.open('editRevenu');
    setTimeout(() => $('edit-rev-nom')?.focus(), 300);
  });
}
window.ouvrirEditRevenu = ouvrirEditRevenu;

function renderCatRevenus(selected, isEdit) {
  const prefix = isEdit ? 'edit-' : '';
  const el = $(prefix + 'rev-cat-select');
  if (!el) return;
  el.innerHTML = CATS_REVENUS.map(c =>
    `<option value="${c}" ${c===selected?'selected':''}>${c}</option>`
  ).join('');
}
window.renderCatRevenus = renderCatRevenus;

window.sauvegarderNewRevenu = async function() {
  const nomEl     = document.getElementById('rev-nom');
  const montantEl = document.getElementById('rev-montant');
  const catEl     = document.getElementById('rev-cat-select');
  ...
  if (!nom) { Toast.error('Indiquez un nom'); nomEl?.focus(); return; }
  if (!montant || montant <= 0) { Toast.error('Indiquez un montant'); montantEl?.focus(); return; }
  try {
    await D().addRevenu({ nom, montant, categorie: cat, actif: true });
    Modal.close('newRevenu');
    Toast.success('Revenu ajouté ✓');
    if (typeof renderRevenus === 'function') renderRevenus();
    if (typeof chargerAccueil === 'function') chargerAccueil();
  } catch(e) {
    Toast.error('Erreur enregistrement');
    console.error('[Revenu]', e);
  }
};

window.sauvegarderEditRevenu = async function() {
  const id = parseInt($('edit-rev-id')?.value);
  const nom = $('edit-rev-nom')?.value?.trim();
  const montant = parseMontant($('edit-rev-montant')?.value);
  const cat = $('edit-rev-cat-select')?.value || 'Autre';
  if (!nom) { Toast.error('Indiquez un nom'); return; }
  if (!montant || montant <= 0) { Toast.error('Montant invalide'); return; }
  await D().updateRevenu(id, { nom, montant, categorie: cat });
  Modal.close('editRevenu');
  Toast.success('Revenu mis à jour ✓');
  renderRevenus();
  if (typeof chargerAccueil === 'function') chargerAccueil();
};

window.ouvrirRevenus = function() { Router.go('revenus'); };
...
window.ajouterChargeVariable=async function(){
  ...
  const plafond=parseMontant($('cv-plafond')?.value);
...
window.sauvegarderEditCV=async function(){
  ...
  if(c){
    ...
    c.plafond=parseMontant($('edit-cv-plafond')?.value);
...
window.ajouterBudgetVirtuel=async function(){
  ...
  const obj=parseMontant($('bv-objectif')?.value); const ech=$('bv-echeance')?.value||'';
...
window.renderCagnottesList = renderCagnottesList;
...
window.ajouterCagnotte=async function(){
  ...
  const prov=parseMontant($('new-cag-provision')?.value);
  const obj=parseMontant($('new-cag-objectif')?.value);
...
window.sauvegarderEditCagnotte=async function(){
  ...
  cag.provisionMensuelle=parseMontant($('edit-cag-provision')?.value);
  cag.objectif=parseMontant($('edit-cag-objectif')?.value);
...
function ouvrirSimulation() {
  const simInput=$('sim-amount-input');
  if(simInput){ simInput.value=''; setTimeout(()=>simInput.focus(),300); }
...
async function lancerSimulation() {
  const simInput=$('sim-amount-input');
  const _simMontant=parseMontant(simInput?.value);
  if(_simMontant<=0){ Toast.error('Saisissez un montant'); if(simInput)simInput.focus(); return; }
...
async function registerSW() {
  ...
  try {
    ...
    reg.addEventListener('updatefound',()=>{
      ...
      newSW.addEventListener('statechange',function(){
        ...
        if(this.state==='activated'){
          // Recharger automatiquement quand le nouveau SW est actif
          window.location.reload();
        }
      });
    });

    // Quand le SW actif change → recharger (iOS Safari)
    ...
    navigator.serviceWorker.addEventListener('message',(event)=>{
      if(event.data?.type==='SW_UPDATED') showUpdateBar();
    });

    // Vérifier une mise à jour à chaque ouverture
    reg.update();
...
window.addEventListener('DOMContentLoaded',async()=>{
  ...
  // ── Détection desktop ──
  if(!localStorage.getItem('horizon_force_desktop') &&
     !window.matchMedia('(max-width:768px)').matches &&
     !window.matchMedia('(display-mode:standalone)').matches &&
     !/Mobi|Android/i.test(navigator.userAgent)){
    afficherPageDesktop(); return;
  }

  // ── Paramètre URL depuis la boutique (?action=activate ou ?key=HRZN-...) ──
  const params = new URLSearchParams(window.location.search);
  const urlKey = params.get('key') || params.get('licence') || params.get('code');
  if(urlKey){
    // Pré-remplir le champ de la clé et afficher l'écran d'activation
    const input = $('licence-input');
    if(input){ input.value = urlKey.toUpperCase(); formatLicenceInput(input); }
    // Rester sur l'écran d'activation — ne pas appeler appInit
    return;
  }

  // ── Vérification licence normale ──
  let ok = false;
  try {
    ok = await LICENCE.isActivated();
  } catch(e) {
    console.error('[Licence] Erreur vérification:', e);
  }

  if (ok) {
    try {
      var actEl = document.getElementById('screen-activation');
      ...
      console.error('[DOMContentLoaded] Erreur démarrage:', e);
      _afficherErreurCritique('Erreur au démarrage. Rechargez la page.');
    }
  } else {
    // Afficher l'écran d'activation
    var actEl2 = document.getElementById('screen-activation');
    if (actEl2) actEl2.style.display = 'flex';
  }
});

// ── Page desktop ──
function afficherPageDesktop(){
  document.body.innerHTML=`
  <div style="min-height:100vh;background:#0a0f0c;display:flex;align-items:center;justify-content:center;font-family:'DM Sans',system-ui,sans-serif;padding:24px">
    <div style="max-width:480px;width:100%;text-align:center">
      <svg width="56" height="56" viewBox="0 0 60 60" fill="none" style="margin-bottom:20px">
        <circle cx="30" cy="30" r="27" stroke="#2d6a4f" stroke-width="2.5" fill="none"/>
        <circle cx="30" cy="30" r="18" stroke="#2d6a4f" stroke-width="2.5" fill="none"/>
        <circle cx="30" cy="30" r="10" stroke="#2d6a4f" stroke-width="2.5" fill="none"/>
        <circle cx="30" cy="30" r="4" fill="#2d6a4f"/>
      </svg>
      <div style="font-size:1.6rem;font-weight:800;color:white;margin-bottom:8px">Horizon Budget</div>
      <div style="font-size:.85rem;color:rgba(255,255,255,.45);margin-bottom:32px">par Horizon PME</div>
      <div style="background:#111814;border:1px solid #1e2822;border-radius:20px;padding:28px 24px;margin-bottom:24px">
        <div style="font-size:2rem;margin-bottom:12px">📱</div>
        <div style="font-size:1rem;font-weight:700;color:white;margin-bottom:8px">Application optimisée pour mobile</div>
        <div style="font-size:.8rem;color:rgba(255,255,255,.5);line-height:1.6">
          Horizon Budget est conçu pour être utilisé sur <strong style="color:rgba(255,255,255,.7)">smartphone</strong>.<br>
          Pour une expérience optimale, installez-le sur votre téléphone.
        </div>
      </div>
      <div style="background:#111814;border:1px solid #1e2822;border-radius:16px;padding:18px;margin-bottom:16px">
        <div style="font-size:.72rem;color:rgba(255,255,255,.4);margin-bottom:10px;text-transform:uppercase;letter-spacing:.1em">Lien de l'application</div>
        <div id="desktop-url" style="font-size:.78rem;color:#4ade80;font-family:monospace;word-break:break-all;margin-bottom:12px">${window.location.origin+window.location.pathname}</div>
        <button onclick="navigator.clipboard.writeText(window.location.href).then(()=>{this.textContent='✅ Copié !';setTimeout(()=>this.textContent='📋 Copier le lien',2000)})"
          style="padding:10px 20px;background:rgba(74,222,128,0.1);border:1px solid rgba(74,222,128,0.25);border-radius:10px;color:#4ade80;font-size:.78rem;font-weight:600;cursor:pointer;width:100%">
          📋 Copier le lien
        </button>
      </div>
      <button onclick="localStorage.setItem('horizon_force_desktop','1');document.getElementById('screen-activation')?.style.setProperty('display','none','important');document.getElementById('screen-onboarding')?.style.setProperty('display','none','important');document.getElementById('app')?.style.setProperty('display','flex','important');document.getElementById('screen-accueil')?.classList.add('active');document.getElementById('screen-accueil')?.style.setProperty('display','flex','important');if(window.Router){Router.current='accueil';}if(window.chargerAccueil){chargerAccueil();}"
        style="padding:12px 24px;background:transparent;border:1px solid rgba(255,255,255,.12);border-radius:12px;color:rgba(255,255,255,.4);font-size:.75rem;cursor:pointer;width:100%">
        Continuer quand même sur ordinateur
      </button>
    </div>
  </div>`;
}
...
window.copierLicence=function(){
  const key=_safeStorage.get('horizon_licence_key', null); if(!key)return;
...
/* ==========================================
   FIX MOBILE — Délai tactile
   ========================================== */
// Supprime le délai de 300ms sur iOS pour tous les éléments cliquables
document.addEventListener('DOMContentLoaded', () => {
  // Fast-click : convertir touchend en click immédiat
  function fastClick(el) {
    ...
    el.addEventListener('touchend', (e) => {
      if (touched) {
        touched = false;
        // Ne pas déclencher si scroll en cours
        e.preventDefault();
        el.click();
      }
    });
  }
  // Appliquer sur la nav
  document.querySelectorAll('.nav-item, .nav-add').forEach(fastClick);
});
