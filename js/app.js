...
async function appInit() {
  ...
  // La configuration se fait depuis les Réglages
  startApp();
}
...
async function appliquerMiseAJour() {
  console.log('[Version] Application de la mise à jour...');

  // 1. Purger tous les caches
  if ('caches' in window) {
    const keys = await caches.keys();
    await Promise.all(keys.map(k => caches.delete(k)));
    console.log('[Version] Caches purgés:', keys);
  }

  // 2. Forcer le nouveau SW à prendre le contrôle
...
window.promptRevenus=async function(){
  const cur=await D().getSetting('revenus','');
  const v=prompt('Revenus mensuels nets (€) :', cur||''); if(v===null)return;
  const n=parseMontant(v); if(!n||n<=0){Toast.error('Montant invalide');return;}
  await D().setSetting('revenus',n); Toast.success('Revenus mis à jour ✓');
